...
export function initHlsPlayer(): void {
  ...
  function startSubtitleWatchdog(
    ...
    _subtitleWatchdogHls = hlsInstance;
    ...
    _subtitleWatchdogInterval = setInterval(() => {
      ...
      // Ensure HLS.js subtitle track stays selected.
      // Only re-activate if track was actually lost (subtitleTrack === -1).
      // Avoid aggressive toggle (subtitleTrack = -1 -> id) which causes flickering.
      if (_subtitleWatchdogHls) {
        const hls = _subtitleWatchdogHls;
        const tracks = hls.subtitleTracks;
        logger.debug("player", "Watchdog check", {
          hasTracks: !!tracks,
        ...
        if (tracks && tracks.length > 0) {
          ...
          if (hls.subtitleTrack === -1 || hls.subtitleDisplay !== true) {
            logger.warn(
              "player",
              "Subtitle watchdog: re-activating lost track selection",
              `subtitleTrack=${hls.subtitleTrack} -> ${targetTrack.id}`,
              `subtitleDisplay=${hls.subtitleDisplay}`,
            );
            hls.subtitleDisplay = true;
            hls.subtitleTrack = targetTrack.id;
            // NOTE: Removed aggressive toggle (subtitleTrack = -1 -> id) that caused flickering.
            // HLS.js will reload fragments automatically when track is set.
          } else if (hls.subtitleTrack !== targetTrack.id) {
            // Track changed (e.g. user selected different language) - just ensure display is on
            hls.subtitleDisplay = true;
          }
        }
      }
  ...
  function connect() {
    logger.info("player", "connect() called");
    hideError();
