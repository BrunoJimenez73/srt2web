...
import type { WhisperModel, Language, Device, TtsEngine, SubtitleFormat, EncoderMode, AudioCodec, VideoCodec } from '../types/api';
...
export function collectConfigFromUI(): Partial<Config> {
  ...
  const configModules: ModulesConfig = {
    ...
    transcriber: {
      ...
      model: ((document.getElementById('whisper-model') as HTMLSelectElement)?.value || DEFAULTS.WHISPER_MODEL) as WhisperModel,
      language: ((document.getElementById('whisper-lang') as HTMLSelectElement)?.value || DEFAULTS.WHISPER_LANGUAGE) as Language,
      device: ((document.getElementById('whisper-device') as HTMLSelectElement)?.value || 'auto') as Device,
      beam_size: 2,
    },
    translator: {
      enabled: (document.getElementById('translator-enabled') as HTMLInputElement)?.checked ?? true,
      source_lang: ((document.getElementById('translator-source') as HTMLSelectElement)?.value || DEFAULTS.WHISPER_LANGUAGE) as Language,
      target_lang: ((document.getElementById('translator-target') as HTMLSelectElement)?.value || DEFAULTS.TRANSLATE_TARGET) as Language,
    },
    tts_engine: {
      enabled: (document.getElementById('tts-enabled') as HTMLInputElement)?.checked ?? true,
      engine: ((document.getElementById('tts-engine') as HTMLSelectElement)?.value || 'edge-tts') as TtsEngine,
      ...
      device: ((document.getElementById('tts-device') as HTMLSelectElement)?.value || 'auto') as Device,
    },
    subtitle_generator: {
      enabled: (document.getElementById('subtitle-enabled') as HTMLInputElement)?.checked ?? true,
      format: ((document.getElementById('subtitle-format') as HTMLSelectElement)?.value || DEFAULTS.SUBTITLE_FORMAT) as SubtitleFormat,
    ...
    video_muxer: {
      ...
      encoder_mode: ((document.getElementById('hls-encoder') as HTMLSelectElement)?.value || 'auto') as EncoderMode,
      ...
      audio_codec: ((document.getElementById('video-muxer-engine') as HTMLSelectElement)?.value === 'webrtc'
        ...
        : (document.getElementById('hls-audio-codec') as HTMLSelectElement)?.value) as AudioCodec || 'aac',
      audio_bitrate: (document.getElementById('hls-audio-bitrate') as HTMLSelectElement)?.value || '192k',
      audio_samplerate: '48000',
      video_codec: ((document.getElementById('webrtc-video-codec') as HTMLSelectElement)?.value as VideoCodec | undefined),
