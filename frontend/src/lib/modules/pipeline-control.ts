...
export function setupEventListeners(): void {
  ...
  document.getElementById('tts-engine')?.addEventListener('change', (e) => {
    ...
    const edgeGroup = document.getElementById('tts-voice-edge-group') as HTMLDivElement;
    const piperGroup = document.getElementById('tts-voice-piper-group') as HTMLDivElement;
...
function setupCopyButtons(): void {
  document.getElementById('btn-copy-emision')?.addEventListener('click', () => {
    ...
    if (urlEl?.textContent) {
      copyToClipboard(urlEl.textContent).then(() => showToast('URL de emisión copiada', 'success')).catch(() => showToast('Error al copiar URL', 'error'));
    }
  });
  ...
  document.getElementById('btn-copy-stream')?.addEventListener('click', () => {
    ...
    if (urlEl?.textContent) {
      copyToClipboard(urlEl.textContent).then(() => showToast('URL del stream copiada', 'success')).catch(() => showToast('Error al copiar URL', 'error'));
    }
  });
  ...
  document.getElementById('btn-copy-player')?.addEventListener('click', () => {
    ...
    if (urlEl) {
      ...
      if (url) copyToClipboard(url).then(() => showToast('URL del player copiada', 'success')).catch(() => showToast('Error al copiar URL', 'error'));
    }
  });
}
