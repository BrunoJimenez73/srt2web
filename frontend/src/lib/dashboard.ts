...
export function updateUrls(ip: string | null): void {
  if (!config || !ip) return;
  ...
  const urlRtmp = document.getElementById('url-rtmp');
  const urlStream = document.getElementById('url-stream');
  const urlPlayer = document.getElementById('url-player');
  const urlItemSrt = document.getElementById('url-item-srt');
  const urlItemRtmp = document.getElementById('url-item-rtmp');

  if (urlSrt && urlItemSrt) {
    if (inputType === 'srt' && config.input?.srt) {
      const srtPort = config.input.srt.listen_port || 9000;
      const srtMode = config.input.srt.mode || 'listener';
      urlSrt.textContent = getSRTUrl(useIp, srtPort, srtMode);
      urlItemSrt.style.display = 'flex';
    } else {
      urlItemSrt.style.display = 'none';
    }
  }
  
  if (urlRtmp && urlItemRtmp) {
    if (inputType === 'rtmp' && config.input?.rtmp) {
      const rtmpUrl = config.input.rtmp.url || 'rtmp://localhost/live/stream';
      let displayUrl = rtmpUrl;
      if (!rtmpUrl.includes(':1935')) {
        const urlParts = rtmpUrl.replace('rtmp://', '').split('/');
        const host = urlParts[0].split(':')[0];
        const path = urlParts.slice(1).join('/');
        displayUrl = `rtmp://${host}:1935/${path}`;
      }
      urlRtmp.textContent = displayUrl;
      urlItemRtmp.style.display = 'flex';
    } else {
      urlItemRtmp.style.display = 'none';
    }
  }
  
  if (urlStream) urlStream.textContent = getStreamUrl(useIp, port);
  if (urlPlayer) {
    urlPlayer.href = getPlayerUrl(useIp, port);
    urlPlayer.textContent = getPlayerUrl(useIp, port);
  }
}
