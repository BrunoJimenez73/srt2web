...
function makeModule(name: string, overrides: Partial<ModuleStatus> = {}): ModuleStatus {
  return {
    name,
    state: 'idle',
    enabled: true,
...
function makeConfig(overrides: Partial<Config> = {}): Config {
  return {
    ...
    modules: {
      ...
      transcriber: { enabled: true, model: 'small', language: 'auto', device: 'cpu', beam_size: 2 },
      translator: { enabled: false, source_lang: 'en', target_lang: 'es' },
      subtitle_generator: { enabled: true, format: 'webvtt', use_translated: false, chunk_duration: 10 },
      tts_engine: { enabled: true, engine: 'piper', device: 'cpu', voice: 'en_US-ryan-low', speed: 1.0 },
      audio_mixer: { enabled: true, original_volume: 0.9, tts_volume: 0.9, dubbed_volume: 0.9 },
      video_muxer: {
        enabled: true,
        engine: 'hls',
...
describe('signals - System Metrics', () => {
  ...
  it('systemMetrics calculates chunksPerSec from uptime', () => {
    const status = makeStatus({ chunks_processed: 100, state: 'running' as any });
  ...
  it('systemMetrics uses avg_processing_time_ms for chunksPerSec when available', () => {
    const status = makeStatus({ chunks_processed: 10, state: 'running' as any });
...
describe('signals - Throughput', () => {
  ...
  it('updateStatus records throughput from uptime when no avg_processing_time_ms', () => {
    const status = makeStatus({ chunks_processed: 20, state: 'running' as any });
  ...
  it('updateStatus keeps only last 10 samples', () => {
    for (let i = 0; i < 15; i++) {
      const status = makeStatus({ chunks_processed: i + 1, state: 'running' as any });
...
describe('signals - Logging', () => {
  ...
  it('addLog keeps only last 500 logs', () => {
    for (let i = 0; i < 600; i++) {
      addLog('INFO', `Log ${i}`);
    }
    expect(pipelineLogs.value.length).toBe(500);
    expect(pipelineLogs.value[0].message).toBe('Log 100'); // First 100 were trimmed
  });
