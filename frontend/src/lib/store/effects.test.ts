...
// Mock DOM elements
function createMockEl(id: string, tag: string = 'span', parent: HTMLElement | null = null): HTMLElement {
...
describe('effects - Status Effect', () => {
  ...
  beforeEach(async () => {
    ...
    statusDot = createMockEl('status-dot', 'span', container) as HTMLSpanElement;
    statusText = createMockEl('status-text', 'span', container) as HTMLSpanElement;
    btnStart = createMockEl('btn-start', 'button', container) as HTMLButtonElement;
    btnStop = createMockEl('btn-stop', 'button', container) as HTMLButtonElement;
  ...
  afterEach(async () => {
    ...
    // Reset signals
    const { pipelineStatus, pipelineLogs, throughputHistory, pipelineConfig, wsConnected, connectionMode } = await import('./signals');
    pipelineStatus.value = null;
...
describe('effects - Module Indicators', () => {
  ...
  it('module indicators are inactive when pipeline is stopped', async () => {
    ...
    updateStatus({
      ...
      chunks_processed: 0,
      modules: [
        { name: 'transcriber', state: 'idle', enabled: true, last_process_time_ms: 0, processed_chunks: 0 },
      ],
    });
    await new Promise(r => setTimeout(r, 50));

    expect(document.getElementById('indicator-whisper')?.classList.contains('active')).toBe(false);
  });
  ...
  it('module indicators are active when pipeline is running and module is enabled', async () => {
    ...
    updateStatus({
      ...
      chunks_processed: 5,
      modules: [
        { name: 'transcriber', state: 'running', enabled: true, last_process_time_ms: 150, processed_chunks: 5 },
        { name: 'translator', state: 'idle', enabled: false, last_process_time_ms: 0, processed_chunks: 0 },
      ],
    });
    await new Promise(r => setTimeout(r, 50));

    expect(document.getElementById('indicator-whisper')?.classList.contains('active')).toBe(true);
    expect(document.getElementById('indicator-translate')?.classList.contains('active')).toBe(false);
  });
});
...
describe('effects - System Metrics', () => {
  ...
  it('CPU metric updates with status data', async () => {
    ...
    const cpuValue = document.getElementById('metric-cpu-value') as HTMLSpanElement;
  ...
  it('memory metric updates correctly', async () => {
    ...
    const memBar = document.getElementById('metric-memory-bar') as HTMLDivElement;
    const memValue = document.getElementById('metric-memory-value') as HTMLSpanElement;
    const memPercent = document.getElementById('metric-memory-percent') as HTMLSpanElement;
  ...
  it('throughput metric updates from throughputAvg', async () => {
    ...
    const tpValue = document.getElementById('metric-throughput-value') as HTMLSpanElement;
...
describe('effects - Module Metrics', () => {
  ...
  beforeEach(async () => {
    ...
    for (const name of ['transcriber', 'translator', 'tts_engine', 'subtitle_generator', 'audio_mixer', 'video_muxer', 'input']) {
  ...
  it('module time shows -- when not running and no data', async () => {
    ...
    const timeEl = document.getElementById('module-time-transcriber') as HTMLSpanElement;
  ...
  it('module time shows actual last_process_time_ms', async () => {
    ...
    updateStatus({
      ...
      chunks_processed: 5,
      modules: [
        { name: 'transcriber', state: 'running', enabled: true, last_process_time_ms: 250, processed_chunks: 5 },
      ],
    });
    await new Promise(r => setTimeout(r, 50));
  ...
  it('module time shows seconds when >= 1000ms', async () => {
    ...
    updateStatus({
      ...
      chunks_processed: 5,
      modules: [
        { name: 'transcriber', state: 'running', enabled: true, last_process_time_ms: 1500, processed_chunks: 5 },
      ],
    });
    await new Promise(r => setTimeout(r, 50));
  ...
  it('module chunks shows processed_chunks', async () => {
    ...
    updateStatus({
      ...
      chunks_processed: 10,
      modules: [
        { name: 'transcriber', state: 'running', enabled: true, last_process_time_ms: 100, processed_chunks: 10 },
      ],
    });
    await new Promise(r => setTimeout(r, 50));

    const chunksEl = document.getElementById('module-chunks-transcriber') as HTMLSpanElement;
  ...
  it('GPU badge is visible when using_gpu is true', async () => {
    ...
    const badge = document.getElementById('gpu-badge-transcriber') as HTMLSpanElement;
...
describe('effects - Pipeline Indicator', () => {
  ...
  it('pipeline indicator is not active when stopped', async () => {
    ...
    const indicator = document.getElementById('pipeline-indicator') as HTMLSpanElement;
...
describe('effects - Remote Mode', () => {
  ...
  it('remote config is hidden in local mode', async () => {
    ...
    const remoteConfig = document.getElementById('remote-config') as HTMLDivElement;
    const btnLocal = document.getElementById('btn-mode-local') as HTMLButtonElement;
    const btnRemote = document.getElementById('btn-mode-remote') as HTMLButtonElement;
