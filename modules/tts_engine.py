...
class TTSEngine(BaseModule):
    ...
    def _init_piper(self):
        ...
        self._piper_voice = PiperVoice.load(model_path, config_path, use_cuda=('CUDAExecutionProvider' in providers))
    ...
    def _ensure_piper_model(self, voice_name: str) -> tuple[str, str]:
        ...
        logger.info(f"Downloading Piper TTS model '{voice_name}' (this happens only once)...")
        ...
        if len(parts) >= 3:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to download Piper model from {base_url}: {e}")
        else:
    ...
    def _run_edge_tts(self, text: str, output_wav: str):
        ...
        async def _generate():
            communicate = edge_tts.Communicate(text, self._voice_model)
    ...
    def _run_piper_tts(self, text: str, output_wav: str):
        ...
        import wave
        if not self._piper_voice: return
