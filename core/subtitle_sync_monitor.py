...
class SubtitleSyncMonitor:
    ...
    def check_sync(self, audio_timestamp_ms: float, subtitle_timestamp_ms: float) -> float:
        ...
        if self.drift_history:
            self.smoothed_drift = self.smoothing_factor * self.smoothed_drift + (1 - self.smoothing_factor) * drift
        ...
        if abs(self.smoothed_drift) > self.threshold_ms:
            ...
            self.logger.warning(
                f"Drift detected: {self.smoothed_drift:.1f}ms, " f"applying correction: {correction:.3f}"
            )
            return correction
        else:
