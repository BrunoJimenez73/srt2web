...
from typing import Optional, Literal, Dict, List, Any, FrozenSet
...
class ServerConfig(BaseModel):
    ...
    rate_limit_rpm: int = Field(
        default=600, ge=1, le=10000, description="Límite de peticiones por minuto"
    )
    max_request_size_mb: int = Field(
        default=100, ge=1, le=1000, description="Tamaño máximo de request en MB"
    )
...
class SRTInputConfig(BaseModel):
    ...
    listen_port: int = Field(
        default=9000, ge=1, le=65535, description="Puerto de escucha SRT"
    )
    mode: Literal["listener", "caller"] = Field(
        default="listener", description="Modo SRT"
    )
    latency_ms: int = Field(
        default=200, ge=0, le=5000, description="Latencia SRT en ms"
    )
    caller_address: str = Field(
        default="", description="Dirección del caller en modo caller"
    )
    chunk_duration_sec: int = Field(
        default=10, ge=1, le=60, description="Duración de chunk en segundos"
    )
...
class RTMPInputConfig(BaseModel):
    ...
    listen_port: int = Field(
        default=1935, ge=1, le=65535, description="Puerto de escucha RTMP"
    )
    ...
    mode: Literal["listener", "pull"] = Field(
        default="listener", description="Modo RTMP"
    )
    chunk_duration_sec: int = Field(
        default=10, ge=1, le=60, description="Duración de chunk en segundos"
    )
...
class FileInputConfig(BaseModel):
    ...
    speed: float = Field(
        default=1.0, ge=0.1, le=10.0, description="Velocidad de reproducción"
    )
    chunk_duration_sec: int = Field(
        default=10, ge=1, le=60, description="Duración de chunk en segundos"
    )
...
class InputConfig(BaseModel):
    ...
    type: InputTypeEnum = Field(
        default=InputTypeEnum.SRT, description="Tipo de entrada"
    )
    srt: SRTInputConfig = Field(
        default_factory=SRTInputConfig, description="Configuración SRT"
    )
    rtmp: RTMPInputConfig = Field(
        default_factory=RTMPInputConfig, description="Configuración RTMP"
    )
    file: FileInputConfig = Field(
        default_factory=FileInputConfig, description="Configuración archivo"
    )
...
class WebOutputConfig(BaseModel):
    ...
    segment_duration: int = Field(
        default=4, ge=1, le=30, description="Duración de segmento HLS en segundos"
    )
    list_size: int = Field(
        default=6, ge=1, le=20, description="Número de segmentos en manifiesto"
    )
    audio_offset_ms: int = Field(
        default=0, ge=-1000, le=1000, description="Offset de audio en ms"
    )
    encoder_mode: EncoderModeEnum = Field(
        default=EncoderModeEnum.AUTO, description="Modo de encoder de video"
    )
...
class RTMPOutputConfig(BaseModel):
    ...
    url: str = Field(
        default="rtmp://localhost/live/stream", description="URL de destino RTMP"
    )
    ...
    video_codec: VideoCodecEnum = Field(
        default=VideoCodecEnum.H264, description="Códec de video"
    )
    preset: str = Field(default="medium", description="Preset de calidad")
    audio_codec: AudioCodecEnum = Field(
        default=AudioCodecEnum.AAC, description="Códec de audio"
    )
    encoder_mode: EncoderModeEnum = Field(
        default=EncoderModeEnum.AUTO, description="Modo de encoder de video"
    )
...
class SRTOutputConfig(BaseModel):
    ...
    mode: Literal["listener", "caller"] = Field(
        default="caller", description="Modo SRT"
    )
    latency_ms: int = Field(
        default=200, ge=0, le=5000, description="Latencia SRT en ms"
    )
...
class RecordingOutputConfig(BaseModel):
    ...
    output_path: str = Field(
        default="./output/recording.mp4", description="Ruta del archivo de salida"
    )
    format: Literal["mp4", "mkv", "webm"] = Field(
        default="mp4", description="Formato del contenedor"
    )
    ...
    quality_mode: Literal["crf", "cbr"] = Field(
        default="cbr", description="Modo de calidad"
    )
    audio_codec: str = Field(
        default="copy", description="Codec de audio (aac, opus, copy)"
    )
    audio_bitrate: str = Field(default="128k", description="Bitrate de audio")
    split_mode: Literal["none", "time", "size"] = Field(
        default="none", description="Modo de split"
    )
    split_value: int = Field(default=600, description="Valor de split (segundos o MB)")
    subtitles: Literal["none", "burnt", "vtt"] = Field(
        default="vtt", description="Subtítulos"
    )
...
class NamedOutputEntry(BaseModel):
    ...
    config: dict = Field(
        default_factory=dict, description="Configuración específica del tipo de salida"
    )
...
class OutputConfig(BaseModel):
    ...
    type: OutputTypeEnum = Field(
        default=OutputTypeEnum.WEB, description="Tipo de salida por defecto (legacy)"
    )
    web: WebOutputConfig = Field(
        default_factory=WebOutputConfig, description="Configuración Web/HLS"
    )
    hls: WebOutputConfig = Field(
        default_factory=WebOutputConfig, description="Alias para web"
    )
    rtmp: RTMPOutputConfig = Field(
        default_factory=RTMPOutputConfig, description="Configuración RTMP"
    )
    srt: SRTOutputConfig = Field(
        default_factory=SRTOutputConfig, description="Configuración SRT"
    )
    file: FileOutputConfig = Field(
        default_factory=FileOutputConfig, description="Configuración archivo"
    )
    recording: RecordingOutputConfig = Field(
        default_factory=RecordingOutputConfig, description="Configuración grabación"
    )
    outputs: list[NamedOutputEntry] = Field(
        default_factory=list, description="Lista de salidas activas"
    )
...
class PipelineConfig(BaseModel):
    ...
    chunk_duration_sec: int = Field(
        default=15, ge=1, le=60, description="Duración de chunk en segundos"
    )
    ...
    buffer_size: int = Field(
        default=5, ge=1, le=20, description="Tamaño del buffer de entrada"
    )
    retry_attempts: int = Field(
        default=2, ge=0, le=10, description="Número de reintentos por módulo"
    )
    retry_delay: float = Field(
        default=1.0, ge=0.1, le=10.0, description="Retraso entre reintentos en segundos"
    )
...
class TranscriberConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Módulo habilitado")
    model: ModelSizeEnum = Field(
        default=ModelSizeEnum.TINY, description="Tamaño del modelo Whisper"
    )
    language: LanguageEnum = Field(
        default=LanguageEnum.AUTO, description="Idioma de transcripción"
    )
    device: DeviceEnum = Field(
        default=DeviceEnum.AUTO, description="Dispositivo de cómputo"
    )
    beam_size: int = Field(
        default=2, ge=1, le=10, description="Tamaño de beam para decoding"
    )
...
class TranslatorConfig(BaseModel):
    ...
    source_lang: LanguageEnum = Field(
        default=LanguageEnum.EN, description="Idioma origen"
    )
    target_lang: LanguageEnum = Field(
        default=LanguageEnum.ES, description="Idioma destino"
    )
...
class SubtitleGeneratorConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Módulo habilitado")
    format: SubtitleFormatEnum = Field(
        default=SubtitleFormatEnum.WEBVTT, description="Formato de subtítulos"
    )
...
class TTSEngineConfig(BaseModel):
    ...
    engine: TTSEngineEnum = Field(
        default=TTSEngineEnum.EDGE_TTS, description="Motor TTS a usar"
    )
    device: DeviceEnum = Field(
        default=DeviceEnum.AUTO, description="Dispositivo de cómputo"
    )
...
class AudioMixerConfig(BaseModel):
    ...
    original_volume: float = Field(
        default=0.2, ge=0.0, le=2.0, description="Volumen audio original"
    )
    tts_volume: float = Field(
        default=1.0, ge=0.0, le=2.0, description="Volumen audio TTS"
    )
    dubbed_volume: float = Field(
        default=1.0, ge=0.0, le=2.0, description="Alias para tts_volume"
    )
...
class VideoMuxerConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Módulo habilitado")
    engine: Literal["hls", "webrtc"] = Field(
        default="hls", description="Motor de salida"
    )
    ...
    video_quality: Literal["low", "medium", "high", "ultra"] = Field(
        default="medium", description="Calidad de video"
    )
    video_crf: int = Field(
        default=23, ge=0, le=51, description="CRF para compresión video"
    )
    audio_codec: AudioCodecEnum = Field(
        default=AudioCodecEnum.AAC, description="Códec de audio"
    )
    ...
    video_bitrate: Optional[str] = Field(
        default=None, description="Bitrate de video fijo"
    )
    video_fps: Optional[int] = Field(
        default=None, ge=1, le=120, description="FPS de video"
    )
    video_width: Optional[int] = Field(
        default=None, ge=160, le=7680, description="Ancho de video"
    )
    video_height: Optional[int] = Field(
        default=None, ge=90, le=4320, description="Alto de video"
    )
    webrtc_audio_codec: Optional[AudioCodecEnum] = Field(
        default=None, description="Códec audio WebRTC"
    )
    webrtc_audio_bitrate: Optional[str] = Field(
        default=None, description="Bitrate audio WebRTC"
    )
    audio_sample_rate: Optional[int] = Field(
        default=None, ge=8000, le=96000, description="Sample rate de audio"
    )
...
class ModulesConfig(BaseModel):
    ...
    transcriber: TranscriberConfig = Field(
        default_factory=TranscriberConfig, description="Transcriptor Whisper"
    )
    translator: TranslatorConfig = Field(
        default_factory=TranslatorConfig, description="Traductor"
    )
    ...
    tts_engine: TTSEngineConfig = Field(
        default_factory=TTSEngineConfig, description="Motor TTS"
    )
    audio_mixer: AudioMixerConfig = Field(
        default_factory=AudioMixerConfig, description="Mezclador de audio"
    )
    video_muxer: VideoMuxerConfig = Field(
        default_factory=VideoMuxerConfig, description="Muxer de video"
    )
...
class SRT2WebConfig(BaseModel):
    ...
    server: ServerConfig = Field(
        default_factory=ServerConfig, description="Configuración del servidor"
    )
    input: InputConfig = Field(
        default_factory=InputConfig, description="Configuración de entrada"
    )
    output: OutputConfig = Field(
        default_factory=OutputConfig, description="Configuración de salida"
    )
    pipeline: PipelineConfig = Field(
        default_factory=PipelineConfig, description="Configuración del pipeline"
    )
    modules: ModulesConfig = Field(
        default_factory=ModulesConfig, description="Configuración de módulos"
    )
    output_dir: OutputDirConfig = Field(
        default_factory=OutputDirConfig, description="Directorio de salida"
    )
    ...
    def validate_module_dependencies(self) -> "SRT2WebConfig":
        ...
        if (
            self.modules.subtitle_generator.enabled
            and not self.modules.translator.enabled
        ):
