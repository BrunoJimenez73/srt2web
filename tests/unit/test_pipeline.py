...
class TestPipeline:
    ...
    def test_stop_pipeline(self):
        ...
        data_source = Mock(return_value=None)
        pipeline.start(data_source)
        
        pipeline.stop()
        
        assert pipeline.state == PipelineState.IDLE
        assert module._stop_called
    ...
    def test_chunk_index_tracking(self):
        ...
        data_items = [
            PipelineData(chunk_index=i) for i in range(3)
        ]
        
        for data in data_items:
            pipeline._run_loop(lambda: data)
        
        # Note: We can't test _run_loop directly without starting the pipeline
        # This is a placeholder for the chunk index logic
        assert pipeline._chunk_index == 0  # Starts at 0, increments in loop
