...
def mock_output_sink():  # type: ignore
    ...
    sink.get_status = Mock(return_value={"state": "running", "enabled": True, "processed_chunks": 10})
...
def mock_composite_output():  # type: ignore
    ...
    mock_output.get_stream_info = Mock(return_value={"type": "web", "master_url": "/hls/master.m3u8"})

    composite.get_all_output_statuses = Mock(
        return_value=[
            {
                "name": "web_output",
                "type": "web",
...
class TestOutputFactory:
    ...
    def test_create_recording_output(self) -> None:
        ...
        config = {"output_path": "./output/test.mp4", "format": "mp4", "codec": "copy"}
