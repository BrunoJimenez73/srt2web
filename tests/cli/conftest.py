...
def mock_api():
    ...
    for method in [
        "get_status", "get_health", "start_pipeline", "stop_pipeline",
        "restart_pipeline", "get_config", "update_config", "update_chunk",
        "get_modules", "toggle_module", "get_module_debug",
        "get_outputs", "get_available_outputs", "add_output", "remove_output",
        "toggle_output", "update_output",
        "get_input_info", "control_input",
        "get_network_info", "get_available",
        "get_presets", "save_preset", "apply_preset", "delete_preset",
        "login", "get_recordings", "delete_recording", "health_check",
        "close",
    ]:
    ...
    api.get_status.return_value = MagicMock(
        ...
        strategy="thread_parallel", modules=[], system={},
        network={}, sync={}, input_receiving=False, input_info={},
    )
    api.get_health.return_value = MagicMock(
        status="ok", uptime_seconds=0, memory_mb=0, memory_percent=0,
        chunks_processed=0, pipeline_state="stopped", modules={},
        input={}, output={},
    )
