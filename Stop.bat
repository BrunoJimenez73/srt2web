...
if %errorlevel% equ 0 (
    for /f "tokens=5" %%A in ('netstat -ano ^| findstr :9999 ^| findstr LISTENING') do (
        ...
        powershell -Command "Write-Host '  [OK] Puerto 9999 liberado.' -ForegroundColor Green"
    )
) else (
    powershell -Command "Write-Host '  [INFO] Puerto 9999 ya libre.' -ForegroundColor White"
)
