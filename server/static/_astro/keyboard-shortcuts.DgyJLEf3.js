...
const s = [
  {
    ...
    ctrlKey: !0,
    handler: (e) => {
      e.preventDefault(), k(), r("Configuración guardada (Ctrl+S)", "info");
    },
  ...
  },
  {
    ...
    handler: (e) => {
      ...
      t === "INPUT" ||
        ...
        _(
          async () => {
            const { pipelineStatus: a } = await Promise.resolve().then(() => C);
            return { pipelineStatus: a };
          },
          void 0,
        ).then(({ pipelineStatus: a }) => {
          a.value?.state === "running" ? D() : w();
        }));
    },
