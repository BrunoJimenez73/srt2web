...
const h = f.AUTH_TOKEN;
function d() {
  return typeof window > "u" ? null : localStorage.getItem(h);
}
function S(s) {
  typeof window > "u" ||
    (s ? localStorage.setItem(h, s) : localStorage.removeItem(h));
}
function m() {
...
function g(s = "/ws/logs") {
  return `${window.location.protocol === "https:" ? "wss:" : "ws:"}//${
    window.location.host
  }${s}`;
}
async function w(s, t = {}, e = 15e3) {
  const n = d(),
    a = new Headers(t.headers);
  n && a.set("Authorization", `Bearer ${n}`);
  const r = new AbortController(),
    c = setTimeout(() => r.abort(), e);
  try {
    return await fetch(s, { ...t, headers: a, signal: r.signal });
  } finally {
    clearTimeout(c);
  }
}
...
async function i(s, t, e, n = 15e3) {
  ...
  const o = await w(c, l, n);
  if (!o.ok) {
    const u = await o.text().catch(() => o.statusText);
    throw new A(`${o.status} ${o.statusText}: ${u}`, o.status, o.statusText);
  }
  return o.json();
}
async function x() {
  return i("GET", "/api/config");
}
async function y(s) {
  return i("POST", "/api/config/chunk", { chunk_duration_sec: s });
}
async function E() {
  return i("POST", "/api/start");
}
async function b() {
  return i("POST", "/api/stop");
}
async function C() {
  return i("GET", "/api/status");
}
const k = {
...
class _ {
  ...
  constructor(t, e = {}) {
    this.url = t;
    const n = { ...k, ...e };
    (this.maxReconnectAttempts = n.maxReconnectAttempts),
      (this.backoffBase = n.backoffBase),
      (this.maxBackoff = n.maxBackoff),
      (this.jitter = n.jitter),
      (this.authToken = n.authToken ?? null);
  }
  connect() {
