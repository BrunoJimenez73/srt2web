const T = { AUTH_TOKEN: "srt2web_auth_token" },
  S = {
  ...
  c = T.AUTH_TOKEN;
function E() {
  return typeof window > "u" ? null : localStorage.getItem(c);
}
function h(e) {
  typeof window > "u" ||
    (e ? localStorage.setItem(c, e) : localStorage.removeItem(c));
}
function d() {
...
async function _(e, t = {}) {
  const n = E(),
    o = new Headers(t.headers);
  return (
    n && o.set("Authorization", `Bearer ${n}`), fetch(e, { ...t, headers: o })
  );
}
...
async function a(e, t, n) {
  ...
  const s = await _(l, r);
  if (!s.ok) {
    const u = await s.text().catch(() => s.statusText);
    throw new p(`${s.status} ${s.statusText}: ${u}`, s.status, s.statusText);
  }
  return s.json();
}
async function P() {
  return a("GET", "/api/config");
}
async function L(e) {
  return a("POST", "/api/config/chunk", { chunk_duration_sec: e });
}
async function N() {
  return a("POST", "/api/start");
}
async function C() {
  return a("POST", "/api/stop");
}
async function f() {
  return a("GET", "/api/status");
}
class U {
  ws = null;
  url;
