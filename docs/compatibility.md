...
| ---------------- | ------------------------------------------------------------ | ---------------------------------------------------------------- |
| **Core**         | fastapi, uvicorn, websockets, numpy, pydantic, psutil, pydub | `pip install -r config/requirements.txt` (nvidia-ml-py excluded) |
| **CLI/TUI**      | textual, httpx, click, rich, colorama                        | `pip install ".[cli]"`                                           |
| **Dev**          | pytest, mypy, ruff, pre-commit                               | `pip install ".[dev]"`                                           |
| **GPU (MPS)**    | PyTorch with MPS                                             | `pip install torch torchvision torchaudio`                       |
| **GPU (CoreML)** | onnxruntime-silicon                                          | `pip install onnxruntime-silicon`                                |
| **TTS**          | piper-tts, edge-tts                                          | `pip install piper-tts edge-tts`                                 |

Notes:

- `nvidia-ml-py` **does not work** on macOS (no NVIDIA GPU on Apple Silicon). `HardwareMonitor` detects it and disables GPU monitoring gracefully.
- `onnxruntime-gpu` **does not work** on macOS. Use `onnxruntime-silicon` for CoreML acceleration.
- FFmpeg installed via Homebrew: `brew install ffmpeg`
- The `install_Mac.sh` script automates all of the above.

## Troubleshooting

### Windows

- **Error: `python` not found**: Install Python 3.12 from python.org
- **Error: `ffmpeg` not found**: Add FFmpeg to PATH
- **Error: `pip` not found**: Re-install Python with "Add to PATH"

### macOS

- **Error: `brew` not found**: Install Homebrew from brew.sh
- **Error: MPS not available**: Use Apple Silicon only (M1/M2/M3)
- **Error: `pip` fails**: Use `python -m pip` instead of `pip`
- **Error: `nvidia-ml-py` fails**: Expected on macOS (no NVIDIA GPU). Use `grep -v nvidia-ml-py requirements.txt` or `install_Mac.sh`
